...
namespace Backend.Services
{
    public class PokemonService
    {
        ...
        public async Task<List<PokemonModel>> GetPokemonsDetailsAsync(int startId, int limit)
        {
            int id = startId;
            try
            {
                ...
                //foreach (PokemonModel pokemon in results)
                //{
                //    tasks.Add(GetPokemonExpandedDetailsAsync(pokemon));
                //}
        ...
        public async Task<PokemonModel> GetPokemonExpandedDetailsAsync(PokemonModel pokemon)
        {
            try
            {
                ...
                if (pokemon.Entwicklung.Count() == 0)
                {
                    ...
                    using var context = _dbFactory.CreateDbContext();
                    context.Entry(pokemon).CurrentValues.SetValues(pokemon);
                    await context.SaveChangesAsync();
                }
